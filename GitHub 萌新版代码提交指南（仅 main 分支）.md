...
打开 [G](https://github.com/)[it](https://github.com/)[Hu](https://github.com/)[b](https://github.com/)[ 官网](https://github.com/)，注册账号（记住用户名和绑定邮箱）。

### 2. 安装 Git 并配置



* 下载 Git：[Git](https://git-scm.com/)[ 官网](https://git-scm.com/)（下载后默认安装即可）；
...
1. 推送时，用户名输入 GitHub 用户名，密码粘贴刚才的令牌，就能登录成功。

### 问题 2：拉取 / 推送时提示「冲突」（和队友改了同一个文件的同一行）



1. 终端会提示「Automatic merge failed」，并告诉你哪个文件冲突了（比如 `main.py`）；
...
| 命令作用         | 命令代码                  |
| ------------ | --------------------- |
| 克隆仓库（第一次拉取）  | git clone 仓库 HTTPS 链接 |
